        ...
        <div class="card mb-4">
            <div class="card-body">
                <div class="d-flex align-items-center justify-content-between">
                    <h5>BAGIAN : <?= $rencana->bagian; ?></h5>
                    <a href="<?= base_url('aplikasi/rencana/read/') ?>"><i class="fa fa-arrow-left"></i> Back</a>
                </div>
                <hr>
                <h5><?= $rencana->rencana; ?></h5>
                <div class="text-secondary d-flex align-items-center justify-content-between">
                    <p class="small"><?= $rencana->tanggal; ?></p>
                    <p class="small">Status : <span class="badge <?= $rencana->status == 'Selesai' ? 'badge-success' : 'badge-primary' ?>"><?= $rencana->status; ?></span></p>
                </div>
                <hr>
                <?php if ($this->session->id == 3 || $this->session->id == 1) : ?>
                            ...
                            <li class="media m-3">
                                ...
                                <?php if ($this->session->id == 1) : ?>
                                    ...
                                    <div class="dropdown-menu">
                                        <a class="small dropdown-item" href="javascript:;" data-id="<?php echo $data->id ?>" data-id_rencana="<?php echo $data->id_rencana ?>" data-kemajuan="<?php echo $data->kemajuan ?>" data-toggle="modal" data-target="#edit-data">
                                            <i class="fa fa-edit"></i> Edit
                                        </a>
                                        <a class="small dropdown-item" onclick="deleteConfirm('<?php echo base_url('aplikasi/kemajuanrencana/delete/' . $data->id) ?>')" href="#!"><i class="fa fa-trash"></i> Hapus</a>
                                    </div>
                                <?php endif; ?>
                            </li>
                        <?php endforeach; ?>
...
<div aria-hidden="true" aria-labelledby="myModalLabel" role="dialog" tabindex="-1" id="edit-data" class="modal fade">
    <div class="modal-dialog">
        <div class="modal-content">
            <form action="<?php echo base_url('/aplikasi/kemajuanrencana/update') ?>" method="post" enctype="multipart/form-data" role="form">
